...
function extractDateFromText(rawText){
  ...
  const D = U.normalize("NFD").replace(/[\u0300-\u036f]/g,""); // deaccent
  ...
  function push(Y,M,Dd,score,pattern,matched,idx){
    ...
    if(idx!=null){
      ...
      if(lot.test(U.slice(b,a))) s-=0.3; // lot blizina spušta score
    }
    if(String(Y).length===2) s-=0.05;
    cands.push({ iso, score:s, pattern, matched });
  }

  // 1) D.M.YYYY (dozvoli razmake oko separatora)
  D.replace(/\b(\d{1,2})\s*[.\-\/]\s*(\d{1,2})\s*[.\-\/]\s*(\d{2,4})\b/g,(m,d,mo,y,off)=>{ push(y,mo,d,0.78,"DD.MM.YYYY",m,off); return m; });

  // 2) YYYY-MM-DD (i varijante s / .)
  D.replace(/\b(20\d{2})\s*[.\-\/]\s*(\d{1,2})\s*[.\-\/]\s*(\d{1,2})\b/g,(m,y,mo,d,off)=>{ push(y,mo,d,0.9,"YYYY-MM-DD",m,off); return m; });

  // 3) MM/YYYY i YYYY/MM → EOM
  D.replace(/\b(\d{1,2})\s*\/\s*(20\d{2})\b/g,(m,mo,y,off)=>{ push(y,mo,lastDay(y,mo),0.72,"MM/YYYY → EOM",m,off); return m; });
  D.replace(/\b(20\d{2})\s*\/\s*(\d{1,2})\b/g,(m,y,mo,off)=>{ push(y,mo,lastDay(y,mo),0.76,"YYYY/MM → EOM",m,off); return m; });

  // 4) YYYY-MM → EOM
  D.replace(/\b(20\d{2})\s*-\s*(\d{1,2})\b/g,(m,y,mo,off)=>{ push(y,mo,lastDay(y,mo),0.72,"YYYY-MM → EOM",m,off); return m; });

  // 5) DD MON YYYY (EN/HR kratko i dugo; tolerira točku)
  D.replace(/\b(\d{1,2})\s*([A-Z]{3,12})\.?\s*(\d{2,4})\b/g,(m,d,mon,y,off)=>{
    const mm = MONTHS[mon] || MONTHS[mon.replace(/Š|Ž|Ć|Đ|Č/g, c=>({Š:'S',Ž:'Z',Ć:'C',Đ:'D',Č:'C'})[c])] || MONTHS[mon.normalize("NFD").replace(/[\u0300-\u036f]/g,"")];
    if(mm) push(y, mm, d, 0.86, "DD MON YYYY", m, off);
    return m;
  });

  // 6) MON YYYY → EOM (EXP/BBE/MHD optional)
  D.replace(/\b(?:EXP(?:\.|IRY)?|BBE|MHD|BEST\s*BEFORE|USE\s*BY)?\s*([A-Z]{3,12})\.?\s*(20\d{2})\b/g,(m,mon,y,off)=>{
    const mm = MONTHS[mon] || MONTHS[mon.normalize("NFD").replace(/[\u0300-\u036f]/g,"")];
    if(mm) push(y, mm, lastDay(y,mm), 0.8, "MON YYYY → EOM", m, off);
    return m;
  });

  // 7) DDMMyy / DDMMyyyy (zbijeno, bez separatora) — pretpostavimo EU (DDMM)
  D.replace(/\b(\d{2})(\d{2})(\d{2})\b/g,(m,d,mo,y,off)=>{ push(y,mo,d,0.65,"DDMMYY (compact)",m,off); return m; });
  D.replace(/\b(\d{2})(\d{2})(20\d{2})\b/g,(m,d,mo,y,off)=>{ push(y,mo,d,0.68,"DDMMYYYY (compact)",m,off); return m; });

  // 8) YYYYMMDD (zbijeno)
  D.replace(/\b(20\d{2})(\d{2})(\d{2})\b/g,(m,y,mo,d,off)=>{ push(y,mo,d,0.7,"YYYYMMDD (compact)",m,off); return m; });
