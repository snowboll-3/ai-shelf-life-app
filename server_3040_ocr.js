...
/* ===== OCR (tesseract + timeout + fallback) ===== */
async function runOCRFromBase64(b64){
  const stub="Best before 2025/11\nEXP 12NOV25\nLOT A12345";
  try{
    const { createWorker } = require("tesseract.js");
    if(!global.__tw){ 
      global.__tw = await createWorker();
      // radi bržeg starta eng; kasnije možemo dodati +hrv
      await global.__tw.loadLanguage("eng");
      await global.__tw.initialize("eng");
    }
    const buf = Buffer.from(b64,"base64");
    const p = global.__tw.recognize(buf, { user_defined_dpi:"300", tessedit_pageseg_mode:6, preserve_interword_spaces:"1" });
    const timeout = new Promise((_,rej)=>setTimeout(()=>rej(new Error("ocr timeout")), 12000));
    const { data:{ text } } = await Promise.race([p, timeout]);
    if (text && text.trim()) return { text, source:"ocr" };
  }catch(e){ /* ignore */ }
  return { text: stub, source:"fallback" };
}
...
<meta charset="utf-8"><title>Scan 3040 (OCR)</title>
<body style="font-family:system-ui;margin:24px">
  <h1>Scan 3040 (OCR + fallback)</h1>
...
btn.onclick=async()=>{
  ...
  try{
    ...
    const img = await preprocess(f.files[0]);
    const b64 = await blobToDataURL(img);
    out.textContent="Calling /api/ocr-date2…";
    let r = await fetch("/api/ocr-date2",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ image:b64 })});
    let j = await r.json();
    if (!j || !j.date){ // fallback
      const r2 = await fetch("/api/ocr-date",{method:"POST"}); j = await r2.json();
    }
...
async function preprocess(file){
  ...
  ctx.putImageData(img,0,0); return await new Promise(res=>work.toBlob(res,"image/png",0.95));
}
...
const server = http.createServer(async (req,res)=>{
  ...
  if (u === "/api/ocr-date2" && req.method === "POST"){
    try{
      ...
      const { text, source } = await runOCRFromBase64(b64);
      const parsed = extractDateFromText(text||""); parsed.text=text||""; parsed.source=source||"fallback";
...
server.listen(PORT,"127.0.0.1",()=>console.log("✅ 3040 OCR server on http://127.0.0.1:"+PORT));
